...
final class ScreenTimeService {
    ...
    /// `forma-ios.entitlements`, `ScreenTimeMonitor.entitlements`, and the
    /// constant of the same name inside `DeviceActivityMonitorExtension`.
    nonisolated static let appGroupID = "group.jashanveer.habit-tracker-macos"

    /// DeviceActivity schedule + event names. Same string on both sides.
    nonisolated static let socialActivityName = DeviceActivityName("forma.social")
    nonisolated static let socialEventName = DeviceActivityEvent.Name("forma.social.overLimit")
